...
def test_imsi_too_long():
    """
    测试步骤4: IMSI 长度过长（异常值）
    预期: GIDS 应返回错误，提示 IMSI 长度不正确
    """
    print("\n[INFO] ========== 测试步骤4: IMSI 长度过长 ==========")

    test_data = {
        "imsi": "12345678901234567890",  # 长度过长 - 异常值
        "imei": "6258412454025411",
        "manufacturer": "xxx manufacturer",
        "model": "xx model",
        "appType": "1",
    ...
    try:
        ...
        if response.status_code == 200 and resp_json.get('code') == 200:
            print("[FAIL] BUG确认: GIDS 对长度过长的 IMSI 没有进行校验，直接返回成功")
            print("[FAIL] 预期: 应返回错误，提示 IMSI 长度应为15位")
            return False
        else:
            print("[PASS] GIDS 正确返回错误")
...
def test_imsi_special_chars():
    """
    测试步骤5: IMSI 包含特殊字符（异常值）
    预期: GIDS 应返回错误，提示 IMSI 格式不正确
    """
    print("\n[INFO] ========== 测试步骤5: IMSI 包含特殊字符 ==========")

    test_data = {
        "imsi": "68510155565!@#",  # 包含特殊字符 - 异常值
        "imei": "6258412454025411",
        "manufacturer": "xxx manufacturer",
        "model": "xx model",
        "appType": "1",
    ...
    print(f"[INFO] 请求参数: imsi={test_data['imsi']} (包含特殊字符)")
    ...
    try:
        ...
        if response.status_code == 200 and resp_json.get('code') == 200:
            print("[FAIL] BUG确认: GIDS 对包含特殊字符的 IMSI 没有进行校验，直接返回成功")
            print("[FAIL] 预期: 应返回错误，提示 IMSI 格式不正确")
            return False
        else:
            print("[PASS] GIDS 正确返回错误")
...
==================================================================
    ...
    # 步骤4: IMSI 长度过长
    result4 = test_imsi_too_long()
    test_results.append(("步骤4: IMSI长度过长", result4))

    # 步骤5: IMSI 包含特殊字符
    result5 = test_imsi_special_chars()
    test_results.append(("步骤5: IMSI包含特殊字符", result5))
    ...
    # 判断整体结果
    # 注意: 由于当前 GIDS 有 BUG，前5个测试会 FAIL，这是预期的
    # 修复 GIDS 后，所有测试应该 PASS
    all_passed = all(result for _, result in test_results)
    print(f"\n{'[SUCCESS]' if all_passed else '[FAILED]'} ========== 测试完成 ==========")

    if not all_passed:
        print("\n[INFO] 注意: 如果前5个测试失败，说明 GIDS 存在 BUG - 缺少对 IMSI 格式的校验")
